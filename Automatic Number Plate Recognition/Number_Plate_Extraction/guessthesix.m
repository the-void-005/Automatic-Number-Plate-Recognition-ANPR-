...
for l=9:-1:2 % This condition has to be changed accordingly if number plates are other than six characters.
    ...
    if isempty(var) || var == 1 % If no index or one index is found.
        ...
        if Q(index)+Q(index+1) == 10 % If the sum of frequencies with the subsequent bin equals six.
            container=[W(index)-(bsize/2) W(index+1)+(bsize/2)]; % Calculae container and break looping
            break;                                               % for more values.
        elseif Q(index)+Q(index-1) == 10 % If the sum of frequencies with the previous bin equals six.
